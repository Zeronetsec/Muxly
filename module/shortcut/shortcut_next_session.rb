...
module ShortcutNextSession
    def self.execute(args)
        ...
        lines.reject! {
            |line| line =~ /^\s*shortcut\.next-session\s*=/
        }

        lines << "shortcut.next-session = #{shortcut_key}\n"
        File.write(Variable.PropPath, lines.join)
        system("termux-reload-settings")

        SetConf.execute("shortcut-next-session", shortcut_key)
        printf(
            "%s[*] %sChange shortcut next session: %s%s%s\n",
