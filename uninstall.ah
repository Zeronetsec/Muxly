...
if [[ "${__RMBK__}" == true ]]; then
    install::getinstall \
        "command rm -f ${opt}/muxly_*.zip.bak" \
        "Removing all backup..."
fi

install::getinstall \
    "command rm -rf ${opt}/muxly" \
    "Removing: ${GG}${opt}/muxly${N}"

install::getinstall \
    "command rm -f ${bin}/muxly" \
    "Removing: ${GG}${bin}/muxly${N}"

if [[ "${__NRMCFG__}" == false ]]; then
    install::getinstall \
        "command rm -rf ${HOME}/.config/muxly" \
        "Removing: ${GG}${HOME}/.config/muxly${N}"
fi

echo -e "${GG}[+] ${N}Muxly removed"
